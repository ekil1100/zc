...
def zig_transition():
    ...
    with Fixture() as f:
        ...
        assert (
            f.run(["status", "--json"])["data"]["selected_proxies"][0]["proxy"]
            == "REJECT"
        )
        ...
        try:
            ...
            result = f.run(
                ["restart", "-c", target, "--port", port(), "--json"], success=False
            )
            assert result["error"]["code"] == "RESTART_FAILED_ROLLED_BACK", result
            state = f.run(["status", "--json"])["data"]
            assert (
                state["mixed_port"] == mixed
                and state["selected_proxies"][0]["proxy"] == "REJECT"
            ), state
        finally:
            occupied.close()

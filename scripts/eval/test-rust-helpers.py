...
BIN = pathlib.Path(
    sys.argv.pop(1) if len(sys.argv) > 1 else ROOT / "target/release/examples"
).resolve()
...
"""
        ...
        bad = self.matrix(
            text.replace("matched_rule: 'DOMAIN,winner.test'", "matched_rule: MATCH")
        )
    ...
    def perf(self, *args):
        return subprocess.run(
            [BIN / "perf_runner", *args], capture_output=True, text=True, timeout=120
        )
    ...
    def test_perf_real_samples_and_provenance(self):
        ...
        for benchmark in report["benchmarks"]:
            ...
            self.assertTrue(
                all(s["iterations"] == 1 and s["elapsed_ns"] > 0 for s in samples)
            )

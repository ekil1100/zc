...
ORACLE = Path(os.environ.get('DIAG_ORACLE', ROOT / 'target/debug/examples/e2e_ss_udp_oracle'))
...
class ProbeContext(unittest.TestCase):
    def probe_result(self, response=None, greeting_only=False):
        with tempfile.TemporaryDirectory() as home, socket.socket() as listener, socket.socket(socket.AF_INET, socket.SOCK_DGRAM) as relay:
            listener.bind(('127.0.0.1', 0))
            listener.listen()
            listener.settimeout(3)
            relay.bind(('127.0.0.1', 0))
            relay.settimeout(3)
            env = dict(os.environ, HOME=home, XDG_CONFIG_HOME=home, XDG_RUNTIME_DIR=home,
                       ZC_E2E_UDP_DIAGNOSTIC='1')
            start = time.monotonic()
            child = subprocess.Popen([str(ORACLE), 'probe', 'roundtrip', str(listener.getsockname()[1]),
                                      str(relay.getsockname()[1]), 'context-test'],
            ...
            with listener.accept()[0] as control:
                ...
                if not greeting_only:
                    ...
                    control.sendall(b'\x05\x00\x00\x01\x7f\x00\x00\x01' + relay.getsockname()[1].to_bytes(2, 'big'))
...
class HarnessCLI(unittest.TestCase):
    def run_harness(self, ssserver=SSSERVER):
        ...
        child = subprocess.Popen([
            sys.executable, str(ROOT / 'scripts/e2e/diagnose-ss-udp.py'),
            '--zc', str(ZC), '--oracle', str(ORACLE), '--ssserver', str(ssserver),
            '--iterations', '2', '--artifacts', str(artifacts),
        ...
        self.assertTrue((artifacts / 'summary.json').exists(), result.stdout + result.stderr)
    ...
    def test_sigterm_reaps_live_owned_fixtures_and_preserves_failure(self):
        ...
        with (artifacts.parent / 'harness.stdout').open('w') as out, (artifacts.parent / 'harness.stderr').open('w') as err:
            child = subprocess.Popen([
                sys.executable, str(ROOT / 'scripts/e2e/diagnose-ss-udp.py'),
                '--zc', str(ZC), '--oracle', str(ORACLE), '--ssserver', str(SSSERVER),
                '--iterations', '100', '--artifacts', str(artifacts),
            ], stdout=out, stderr=err)
            try:
                deadline = time.monotonic() + 10
                ready = artifacts / '0001/echo-ipv4.stdout'
                while not ready.exists() or 'E2E_UDP_ECHO_READY' not in ready.read_text():
    ...
    def test_failure_then_pass_is_retained_and_final_exit_stays_nonzero(self):
        base = ROOT / 'target/diagnose-ss-udp'
        base.mkdir(parents=True, exist_ok=True)
        fixture = Path(tempfile.mkdtemp(prefix='negative-fixture-', dir=base)) / 'ssserver'
...
''')
        ...
        self.assertTrue(all(c['reaped'] for i in summary['iterations'] for c in i['children']))
    ...
    def test_independent_server_real_socks_echo_loop_and_child_reaping(self):
        ...
        for item in summary['iterations']:
            ...
            self.assertIn('E2E_SS_UDP_PROBE_PASS=roundtrip', (iteration / 'probe-roundtrip.stdout').read_text())
            self.assertIn('created udp association for', (iteration / 'ssserver.stdout').read_text())
            self.assertIn('E2E_UDP_ECHO_PACKET=ipv4:1', (iteration / 'echo-ipv4.stdout').read_text())
            self.assertTrue(all(child['reaped'] for child in item['children']))
            for child in item['children']:
                self.assertNotEqual(child['returncode'], -9, 'Child inherited a blocked termination signal')

...
class Children:
    ...
    def start(self, name, argv):
        ...
        with (self.directory / (name + '.stdout')).open('w') as out, (self.directory / (name + '.stderr')).open('w') as err:
            child = subprocess.Popen(list(map(str, argv)), cwd=self.directory,
                                     env=self.env, stdout=out, stderr=err)
            record = {'name': name, 'pid': child.pid, 'argv': list(map(str, argv)),
                      'started_monotonic_ns': time.monotonic_ns(), 'reaped': False}
...
def echo_count(directory, family):
    return len(re.findall(r'^E2E_UDP_ECHO_PACKET=', read(directory / f'echo-{family}.stdout'), re.M))
...
def iteration(args, directory):
    ...
    env = {'PATH': os.environ.get('PATH', '/usr/bin:/bin'), 'HOME': str(directory / 'home'),
           'XDG_RUNTIME_DIR': str(directory / 'run'), 'XDG_CONFIG_HOME': str(directory / 'config'),
           'XDG_CACHE_HOME': str(directory / 'cache'), 'XDG_STATE_HOME': str(directory / 'state'),
           'TMPDIR': str(directory / 'tmp'), 'NO_PROXY': '*', 'no_proxy': '*', 'NO_COLOR': '1',
    ...
    try:
        ...
        held.pop().close()
        server = children.start('ssserver', [args.ssserver, '-U', '-s', f'127.0.0.1:{server_port}',
                                            '-k', 'e2e-password', '-m', args.cipher, '-v', '--log-without-time'])
        services.append(server)
        wait_until(lambda: alive(server) and 'shadowsocks udp server listening on' in read(directory / 'ssserver.stdout'), 'ssserver UDP')
        ...
        wait_until(lambda: alive(echo4) and 'E2E_UDP_ECHO_READY=ipv4:' in read(directory / 'echo-ipv4.stdout'), 'IPv4 echo')
        echo_port = int(re.search(r'E2E_UDP_ECHO_READY=ipv4:(\d+)', read(directory / 'echo-ipv4.stdout'))[1])
        if echo_port in (0, 7899):
            raise RuntimeError('Invalid echo port')
        echo6 = children.start('echo-ipv6', [args.oracle, 'echo', 'ipv6', str(echo_port)])
        services.append(echo6)
        wait_until(lambda: alive(echo6) and f'E2E_UDP_ECHO_READY=ipv6:{echo_port}' in read(directory / 'echo-ipv6.stdout'), 'IPv6 echo')
...
''')
        config.chmod(0o400)
        result.update(config_sha256=digest(config), ports={'mixed': mixed, 'controller': controller,
                                                         'ssserver': server_port, 'echo': echo_port})
        load = json.loads(children.run('config-load', [args.zc, 'config', 'load', config, '--json']))
        if not load.get('ok') or not load.get('data', {}).get('active'):
            raise RuntimeError('Managed config not active')
        for reservation in held:
            reservation.close()
        held.clear()
        daemon = children.start('zc', [args.zc, 'start', '--foreground', '--port', str(mixed)])
        ...
        def ready():
            ...
            alive(daemon)
            status_index += 1
            value = json.loads(children.run(f'status-{status_index}', [args.zc, 'status', '--json']))
            return value.get('ok') and value.get('data', {}).get('state') == 'running'
        wait_until(ready, 'zc ready descriptor')
        selection = json.loads(children.run('select', [args.zc, 'proxy', 'select', '-g', 'Proxy', '-p', 'ss-target', '--json']))
        if not selection.get('ok') or selection.get('data', {}).get('applied') is not True:
        ...
        for kind in args.kinds:
            ...
            try:
                for service in services:
                    alive(service)
                output = children.run('probe-' + kind, [args.oracle, 'probe', kind, str(mixed), str(echo_port),
                                                        f'rust-{args.cipher}-{kind}'])
                if f'E2E_SS_UDP_PROBE_PASS={kind}' not in output:
                    raise RuntimeError('Missing probe pass marker')
                wait_until(lambda: sum(echo_count(directory, f) for f in ('ipv4', 'ipv6')) == sum(before) + 1, 'echo attestation')
                wait_until(lambda: association_count(directory) == associations + 1, 'ssserver association')
                for _ in range(10):
                    time.sleep(0.05)
                    after = [echo_count(directory, f) for f in ('ipv4', 'ipv6')]
                    if sum(after) != sum(before) + 1 or association_count(directory) != associations + 1:
                        raise RuntimeError('Protocol counters changed during quiet period')
                ...
                probe.update(elapsed_ms=round((time.monotonic() - tick) * 1000, 3),
                             echo_before=before, echo_after=[echo_count(directory, f) for f in ('ipv4', 'ipv6')],
                             associations_before=associations, associations_after=association_count(directory))
        ...
        result['config_unchanged'] = config.exists() and digest(config) == result.get('config_sha256')
...
def main():
    ...
    parser.add_argument('--oracle', type=Path, default=ROOT / 'target/debug/examples/e2e_ss_udp_oracle')
    parser.add_argument('--ssserver', type=Path, default=ROOT / 'target/e2e-fixtures/ssserver')
    parser.add_argument('--artifacts', type=Path, required=True, help='New directory; never overwritten or deleted')
    parser.add_argument('--iterations', type=int, default=10)
    parser.add_argument('--cipher', choices=('aes-128-gcm', 'aes-256-gcm', 'chacha20-ietf-poly1305'), default='aes-128-gcm')
    parser.add_argument('--kinds', nargs='+', choices=('roundtrip', 'roundtrip-domain', 'roundtrip-ipv6'), default=['roundtrip'])
    ...
    summary = {'platform': platform.platform(), 'machine': platform.machine(), 'requested': args.iterations,
               'cipher': args.cipher, 'kinds': args.kinds, 'probe_deadline_seconds': 5,
               'iterations': [], 'passed': 0, 'failed': 0,
               'binaries': {name: {'path': str(getattr(args, name)),
                                   'sha256': digest(getattr(args, name)) if getattr(args, name).is_file() else None}
                            for name in ('zc', 'oracle', 'ssserver')}}
    ...
    for number in range(1, args.iterations + 1):
        ...
        save(args.artifacts / 'summary.json', summary)
        print(f"[DEBUG-udp-harness] iteration={number} ok={result['ok']} artifacts={args.artifacts / result['directory']}", flush=True)
        if result.get('interrupted'):
            break
    print(f"[DEBUG-udp-harness] passed={summary['passed']} failed={summary['failed']} requested={args.iterations} artifacts={args.artifacts}")

...
def source_state():
    ...
    files = {
        ...
        if name
        and (
            name.endswith((".rs", ".zig", ".zon"))
            or name in ("Cargo.toml", "Cargo.lock", "scripts/perf/compare-runtimes.py")
        )
...
def paired(name, operations, iterations, count):
    ...
    for runtime, values in samples.items():
        ...
        result["runtimes"][runtime] = {
            "samples": values,
            "median_ns_per_op": statistics.median(ordered),
            "p95_ns_per_op": ordered[math.ceil(len(ordered) * 0.95) - 1],
        }
    result["rust_over_zig_median"] = (
        result["runtimes"]["rust"]["median_ns_per_op"]
        / result["runtimes"]["zig"]["median_ns_per_op"]
    )
    return result
...
def main():
    ...
    parser.add_argument(
        "--zig", type=Path, default=ROOT / "target/zig-reference/bin/zc"
    )
    parser.add_argument(
        "--build",
        action="store_true",
        help="build both default release candidates and record source hashes across the build",
    )
    ...
    if args.build:
        if binaries != {
            "zig": ROOT / "target/zig-reference/bin/zc",
            "rust": ROOT / "target/release/zc",
        }:
        ...
        with build_log.open("w") as log:
            for command in commands:
                subprocess.run(
                    command, cwd=snapshot, stdout=log, stderr=log, check=True
                )
        ...
        build_evidence = {
            ...
            "cwd": str(snapshot),
            "log": str(build_log),
            "log_sha256": digest(build_log),
            "snapshot_files_sha256": manifest,
            "snapshot_manifest_sha256": hashlib.sha256(
                json.dumps(manifest, sort_keys=True).encode()
            ).hexdigest(),
            "snapshot_changed_during_build": any(
                digest(snapshot / name) != value for name, value in manifest.items()
            ),
        }
        before = source_state()
    report = {
        "schema_version": 1,
        "kind": "exploratory-runtime-comparison",
        ...
        "build_contract": {
            "rust": "cargo build --locked --release --bin zc",
            "zig": "zig build -Doptimize=ReleaseFast --prefix target/zig-reference",
            "note": "--build records build/source binding; without it the caller is responsible for build mode; binary hashes identify supplied candidates",
        },
        "method": {
            "warmup_runs": 1,
            "sample_count": args.samples,
            "clock": "time.perf_counter_ns",
            "order": "alternating runtime order per sample",
            "network": "loopback only",
            "CLI": "config dump --no-override --json; includes process startup, parse/validation, serialization and output capture",
            "connection": "HTTP CONNECT + verified 4KiB echo, new connection per operation",
            "stream": "persistent CONNECT + verified 64KiB echo per operation",
        },
        "fixtures": [],
        "benchmarks": [],
        "omitted": [
            "in-process parser/route microbenchmarks",
            "external DNS",
            "WAN throughput",
            "long-duration soak",
            "formal threshold judgment",
        ],
    }
    signal.signal(
        signal.SIGTERM, lambda *_: (_ for _ in ()).throw(InterruptedError("terminated"))
    )
    try:
        with (
            tempfile.TemporaryDirectory(prefix="zc-runtime-compare-") as work,
            ...
            try:
                for rule_count in (100, 10000):
                    ...
                    fixture.write_text(
                        "mixed-port: 18080\nallow-lan: false\nmode: rule\nrules:\n"
                        + "".join(
                            f"  - DOMAIN,bench-{index}.invalid,DIRECT\n"
                    ...
                    report["benchmarks"].append(
                        paired(
                            f"config_dump_rules_{rule_count}",
                            {name: lambda name=name: dump(name) for name in binaries},
                            3,
                            args.samples,
                        )
                    )
                    ...
                    try:
                        for name, binary in binaries.items():
                            ...
                            while True:
                                ...
                                try:
                                    with tunnel(
                                        ports[name], origin.server_address[1]
                                    ) as client:
                                    ...
                                    if time.monotonic() >= deadline:
                                        raise RuntimeError(
                                            f"{name} startup timed out; see {log_path}"
                                        )
                                    time.sleep(0.02)
                        ...
                        report["benchmarks"].append(
                            paired(
                                f"connect_echo_4k_rules_{rule_count}",
                                {
                                    name: lambda name=name: connection(name)
                                    for name in binaries
                                },
                                args.iterations,
                                args.samples,
                        ...
                        report["benchmarks"].append(
                            paired(
                                f"stream_echo_64k_rules_{rule_count}",
                                {
                                    name: lambda name=name: exchange(
                                        clients[name], payload
                                    )
                                    for name in binaries
                                },
                                args.iterations,
        ...
        report["shared_source_changed_during_run"] = (
            report["provenance_before"]["source_manifest_sha256"]
            != report["provenance_after"]["source_manifest_sha256"]
        )
        if args.build:
            report["snapshot_changed_during_run"] = any(
                digest(snapshot / name) != value for name, value in manifest.items()
            )
        report["binary_sha256_after"] = {
            name: digest(path) for name, path in binaries.items()
        }

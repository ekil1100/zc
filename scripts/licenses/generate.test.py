...
class NoticesTests(unittest.TestCase):
    def test_closure_excludes_dev_but_keeps_build_and_target_edges(self):
        def edge(package, kind):
            return {
                "pkg": package,
                "dep_kinds": [{"kind": kind, "target": "cfg(windows)"}],
            }
        ...
        metadata = {
            "resolve": {
                ...
                "nodes": [
                    ...
                    {
                        "id": "run",
                        "deps": [edge("shared", None), edge("nested-dev", "dev")],
                    },
                    {"id": "build", "deps": []},
                    {"id": "shared", "deps": []},
                    {"id": "dev", "deps": []},
        ...
        self.assertEqual(
            generate.production_closure(metadata), {"root", "run", "build", "shared"}
        )
    ...
    def test_collects_hidden_license_directory(self):
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            (root / ".licenses").mkdir()
            (root / ".licenses/OriginalAuthor-MIT").write_text(
                "Permission is hereby granted"
            )
            self.assertEqual(
                generate.collect_texts(root),
                [(".licenses/OriginalAuthor-MIT", "Permission is hereby granted")],
            )

    def test_recognizes_cc0_but_not_license_link_only(self):
        self.assertTrue(
            generate.has_license_body(
                "CC0 1.0 Universal\nStatement of Purpose\n1. Copyright and Related Rights"
            )
        )
        self.assertFalse(
            generate.has_license_body("See https://opensource.org/licenses/MIT")
        )

    def test_supplement_rejects_changed_source_revision(self):
        with self.assertRaises(ValueError):
            generate.validate_supplement(
                {"revision": "old"},
                {"git": {"sha1": "new"}},
    ...
    def test_supplement_rejects_changed_text(self):
        with self.assertRaises(ValueError):
            generate.validate_supplement(
                {"revision": "rev", "text": "changed", "sha256": "bad"},
                {"git": {"sha1": "rev"}},
                "https://github.com/example/repo",
            )
    ...
    def test_unknown_license_is_reported(self):
        self.assertEqual(
            generate.unknown_license_tokens("Apache-2.0 WITH LLVM-exception OR MIT"), []
        )
        self.assertEqual(
            generate.unknown_license_tokens("MIT AND LicenseRef-New"),
            ["LicenseRef-New"],
        )

...
NOTICE_NAME = re.compile(r"^(licen[cs]e|copying|copyright|notice|authors|patents|unlicense)(?:$|[._-])", re.I)
NATIVE_SUFFIXES = {".c", ".h", ".cc", ".cpp", ".hpp", ".s", ".asm", ".inc", ".pl"}
COMMENT = re.compile(r"/\*.*?\*/|(?:^[ \t]*(?://|\#|;)[^\n]*(?:\n|$))+", re.S | re.M)
LEGAL = re.compile(r"copyright|permission is hereby|redistribution and use|SPDX-License-Identifier|public domain", re.I)
...
def unknown_license_tokens(expression):
    ...
    known = {"MIT", "Apache-2.0", "ISC", "BSD-3-Clause", "MIT-0", "CC0-1.0",
             "LLVM-exception", "Unicode-3.0", "Zlib", "Unlicense", "BSL-1.0",
             "LGPL-2.1-or-later", "AND", "OR", "WITH"}
    return sorted(set(re.findall(r"[A-Za-z0-9.+-]+", expression)) - known)


def has_license_body(text):
    return bool(re.search(r"permission (?:is hereby granted|to use)|redistribution and use|TERMS AND CONDITIONS|Unicode License|CC0 1\.0 Universal", text, re.I))
...
def collect_texts(root):
    ...
    for path in sorted(root.rglob("*")):
        ...
        if NOTICE_NAME.match(path.name) or any(part.lower() in {"licenses", ".licenses", "licences", ".licences"} for part in path.relative_to(root).parts[:-1]):
...
def validate_supplement(entry, vcs, repository):
    ...
    expected = repository.replace("https://github.com/", "https://raw.githubusercontent.com/")
    ...
    with tarfile.open(archive) as crate:
        for relative in sorted(set(paths) | {"Cargo.toml"}):
            ...
            if member is None or member.read() != (directory / relative).read_bytes():
                raise ValueError(f"Registry source differs from locked archive: {directory.name}/{relative}")
...
def generate():
    ...
    for package in sorted(metadata["packages"], key=lambda p: (p["name"], p["version"])):
        ...
        if package["id"] not in closure:
            excluded.append({"name": name, "version": version, "reason": "仅由 dev 边可达"})
            continue
        ...
        paths = [path.rsplit(":", 1)[0] if re.search(r":\d+$", path) else path for path, _ in texts]
        if vcs_path.exists():
        ...
        if unknown:
            issues.append(f"{name} {version}：尚未审核的许可标识 {', '.join(unknown)}。")
        ...
        packages.append({"name": name, "version": version, "license": package["license"],
                         "source": source, "checksum": entry["checksum"], "repository": package["repository"],
                         "vcs": vcs, "features": sorted(nodes[package["id"]]["features"]), "texts": references})
    if used_supplements != set(range(len(supplements))):
        raise ValueError("Unused upstream supplement; update version-pinned snapshots")
    if lock_bytes != (ROOT / "Cargo.lock").read_bytes() or manifest_bytes != (ROOT / "Cargo.toml").read_bytes():
    ...
    report = {"project": f"{project['name']} {project['version']}", "command": command,
              "cargo_lock_sha256": sha256(lock_bytes), "cargo_toml_sha256": sha256(manifest_bytes),
              "production_package_count": len(packages), "unique_text_count": len(documents),
              "issues": issues, "limitations": limitations, "excluded": excluded, "packages": packages}
    output = ["# 第三方许可与版权声明\n\n",
    ...
    return "".join(output).encode(), (json.dumps(report, ensure_ascii=False, indent=2) + "\n").encode(), issues, limitations


def main():
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--check", action="store_true", help="Check committed artifacts without writing them")
    args = parser.parse_args()
    try:
        notices, report, issues, limitations = generate()
        stale = []
        for path, content in [(ROOT / "THIRD_PARTY_NOTICES.md", notices), (HERE / "license-report.json", report)]:
            if args.check:
        ...
        for index, _ in enumerate(issues, 1):
            print(f"ERROR: license metadata/text/identifier finding {index}; see license-report.json", file=sys.stderr)
        for index, _ in enumerate(limitations, 1):
            print(f"REVIEW: scope/coverage limitation {index}; see notices and license-report.json", file=sys.stderr)
        if stale:
            print("ERROR: stale artifacts: " + ", ".join(stale), file=sys.stderr)
        print(f"{'Checked' if args.check else 'Generated'} notices and license report; missing-text findings: {len(issues)}")

...
def service_logs(password, cleanup_errors):
    ...
    try:
        ...
        print(
            f"BEGIN service logs pid={child.pid} utc={datetime.now(timezone.utc).isoformat()}",
            flush=True,
        )
        ...
        if child is not None:
            ...
            try:
                ...
                cleanup_errors.append(
                    f"service log child reap/read failed ({type(error).__name__})"
                )
            finally:
                try:
                    child.stdout.close()
                    ...
                    cleanup_errors.append(
                        f"service log stream close failed ({type(error).__name__})"
                    )
            text = native.diagnostic_text(output, ["-p", password]).encode("utf-8")
            if text:
                print(
                    "EVIDENCE service logs (bounded prefix; may be filtered, buffered or truncated):",
                    flush=True,
                )
                print(
                    text[: LOG_LIMIT - 1].decode("utf-8", errors="ignore"), flush=True
                )
            else:
                print("WARN service logs: no evidence captured", flush=True)
            if status is not None:
                print(
                    f"WARN service logs exited before collection ended (exit {status})",
                    flush=True,
                )
            print(
                f"END service logs pid={child.pid} elapsed={time.monotonic() - started:.3f}s",
                flush=True,
            )
        print(
            "NOTE missing service logs does not rule out authorization/IPC waits; "
            "startup, permissions and privacy filtering limit evidence",
            flush=True,
        )
...
def authorization_window(password):
    ...
    if text:
        print(
            "EVIDENCE authorization window (bounded prefix; may be truncated):",
            flush=True,
        )
        # Reserve space for the probe's status lines as well as the newline.
        print(
            text.encode("utf-8")[: LOG_LIMIT - 1024].decode("utf-8", errors="ignore"),
            flush=True,
        )
    else:
        print("WARN authorization window: no evidence captured", flush=True)
    print(
        f"END authorization window: {outcome}; elapsed={time.monotonic() - started:.3f}s",
        flush=True,
    )
    print(
        "NOTE persistence, permissions and privacy may omit context; without a linked right/request, "
        "origin/GUI cause remains unknown",
        flush=True,
    )
...
def diagnose(keychain_only):
    ...
    print(
        "DIAGNOSTIC ONLY: "
        + ("keychain-only" if keychain_only else "user TrustSettings")
        + "; no Rust or TLS verification",
        flush=True,
    )
    original_search = shlex.split(
        native.command(
            [native.SECURITY, "list-keychains", "-d", "user"],
    ...
    try:
        ...
        native.command(
            [native.SECURITY, "create-keychain", "-p", password, keychain],
            "create owned keychain",
        )
        native.command(
            [native.SECURITY, "unlock-keychain", "-p", password, keychain],
        ...
        with service_logs(password, errors):
            ...
            if keychain_only:
                native.command(
                    [native.SECURITY, "add-certificates", "-k", keychain, certificate],
        ...
        if write_attempted:
            try:
                ...
                print(
                    f"WARN authorization window unavailable ({type(error).__name__})",
                    flush=True,
                )
        ...
        if trust_attempted:
            ...
            cleanup(
                lambda: native.command(
                    [native.SECURITY, "remove-trusted-cert", certificate],
        ...
        if keychain_attempted:
            ...
            def delete_keychain():
                if keychain.exists():
                    native.command(
                        [native.SECURITY, "delete-keychain", keychain],
        ...
        if not errors:
            print(
                "PASS cleanup: owned fixture removed; original search list restored",
                flush=True,
            )
...
def main():
    ...
    parser.add_argument(
        "--target", required=True, choices=sorted(native.TARGETS.values())
    )
    parser.add_argument(
        "--keychain-only",
        action="store_true",
    ...
    print(
        f"DIAGNOSTIC ONLY environment: target={args.target} uid={os.getuid()} euid={os.geteuid()}",
        flush=True,
    )
    ...
    print(
        "DIAGNOSTIC ONLY complete; no TLS/DNS or native nine-case verification",
        flush=True,
    )

...
def check_segment(data, body, commands_end, delay_sections):
    (
        ...
        count,
        flags,
    ) = unpack(body, 0, "II16sQQQQ4I", "segment")
    ...
    for index in range(count):
        (
            ...
            r2,
            r3,
        ) = unpack(body, 72 + index * 80, "16s16sQQ8I", "section")
        name = name.rstrip(b"\0")
        require(owner.rstrip(b"\0") == segment, "section segment name mismatch")
        require(
            vmaddr <= address <= vmaddr + vmsize and size <= vmaddr + vmsize - address,
        ...
        if name in DELAY_SECTIONS:
            ...
            require(
                segment == b"__TEXT" and initprot & maxprot & 4,
                f"{label}: expected __TEXT executable segment",
            )
            require(size > 0, f"{label}: expected nonempty delay section")
            require(
                attributes & 0xFF in (0, 8) and attributes & 0x80000400,
                f"{label}: expected file-backed instructions",
            )
            require(
                any(data[offset : offset + size]), f"{label}: expected nonzero code"
            )
...
def check_imports(data, symtab, commands_end, frameworks):
    ...
    require(
        symoff >= commands_end and stroff >= commands_end,
        "symbol/string table overlaps load commands",
    )
    require(
        symoff + count * 16 <= stroff or stroff + strsize <= symoff,
...
def verify(data):
    ...
    require(
        magic == 0xFEEDFACF,
        "expected thin little-endian 64-bit Mach-O (not fat/32-bit)",
    )
    require(
        cpu in (0x0100000C, 0x01000007),
    ...
    require(
        flags & 0x84 == 0x84 and not flags & 0x100,
    ...
    for _ in range(count):
        ...
        require(
            length >= 8 and length % 8 == 0 and length <= end - offset,
        ...
        if command in (0x32, 0x24, 0x25, 0x2F, 0x30):
            ...
            if command == 0x32:  # LC_BUILD_VERSION
                ...
                require(
                    length == 24 + tools * 8, "invalid build version tool count/size"
                )
            ...
            nameoff, marker, current, compatibility = unpack(
                body, 8, "4I", "dylib command"
            )
            require(24 <= nameoff < length, "invalid dylib path offset")
            require(
                marker != 0x1A741800 or nameoff == 28,
            ...
            for name, expected in FRAMEWORKS.items():
                ...
                if name.encode() == path.rsplit(b"/", 1)[-1] or (
                    name + ".framework"
                ).encode() in path.split(b"/"):
                    require(
                        path == expected, f"{name}: non-system framework path {path!r}"
                    )
                    require(name not in frameworks, f"duplicate framework: {name}")
                    require(
                        command == 0xC and nameoff == 28 and marker == 0x1A741800,
    ...
    for name in DELAY_SECTIONS:
        require(
            name in delay_sections, f"missing delay section: __TEXT,{name.decode()}"
        )
...
def main():
    ...
    parser.add_argument(
        "artifact", type=Path, help="path to the final thin macOS zc executable"
    )
    ...
    try:
        ...
        return 1
    print(
        f"PASS: {args.artifact}: {architecture}, minOS 15.0.0; "

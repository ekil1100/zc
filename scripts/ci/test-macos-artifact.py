...
def fixture(cpu=0x0100000C, minimum=0x000F0000):
    ...
    for path in paths:
        ...
        commands.append(struct.pack("<7I", 0xC, size, 28, 0x1A741800, 0, 0, 8)
                        + (path + b"\0").ljust(size - 28, b"\0"))
    sections = b""
    for name, offset in [(b"__delay_stubs", 4096), (b"__delay_helper", 4112)]:
        sections += struct.pack("<16s16sQQ8I", name, b"__TEXT",
                                0x100000000 + offset, 16, offset, 2,
                                0, 0, 0x80000400, 0, 0, 0)
    commands.append(struct.pack("<II16sQQQQ4I", 0x19, 232, b"__TEXT",
                                0x100000000, 8192, 0, 8192, 5, 5, 2, 0)
                    + sections)
    strings = b"\0"
    symbols = b""
    for name, ordinal in [
    ...
    commands.append(struct.pack("<II16sQQQQ4I", 0x19, 72, b"__LINKEDIT",
                                0x100002000, 4096, 8192,
                                len(symbols + strings), 1, 1, 0, 0))
    header = struct.pack("<8I", 0xFEEDFACF, cpu, 0 if cpu == 0x0100000C else 3,
                         2, len(commands), len(b"".join(commands)), 0x200085, 0)
    return bytearray((header + b"".join(commands)).ljust(4096, b"\0")
                     + bytes.fromhex("1f2003d5") * 8 + bytes(4064)
                     + symbols + strings)
...
class ArtifactCLI(unittest.TestCase):
    ...
    def test_all_three_frameworks_must_be_strong_delayed(self):
        for index in range(3):
            ...
            data = fixture()
            self.check(put(data, command_offset(data, 0xC, index), 0x1B), "missing framework")
    ...
    def test_framework_paths_marker_and_duplicates(self):
        for relative, value, diagnostic in [
            ...
            data = fixture()
            self.check(put(data, command_offset(data, 0xC) + relative, value), diagnostic)
        data = fixture()
    ...
    def test_segment_bounds_and_section_count(self):
        for relative, value, fmt, diagnostic in [
            ...
            data = fixture()
            self.check(put(data, command_offset(data, 0x19) + relative, value, fmt), diagnostic)
        data = fixture()
    ...
    def test_native_imports_cannot_be_removed_defined_weak_or_redirected(self):
        for index in range(8):
            for relative, value, fmt in [
                ...
                with self.subTest(index=index, relative=relative, value=value):
                    self.check(put(fixture(), 8192 + index * 16 + relative, value, fmt),
                               "native import")
    ...
    def test_symbol_table_bounds_and_missing_table(self):
        ...
        self.check(append_command(data, bytes(data[offset:offset + 24])), "duplicate LC_SYMTAB")

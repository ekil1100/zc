...
FRAMEWORKS = ("CoreFoundation.framework/", "Security.framework/", "SystemConfiguration.framework/")
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix="zc-macos-launch-") as temporary:
        ...
        for key in ("HOME", "TMPDIR", "XDG_CONFIG_HOME", "XDG_DATA_HOME", "XDG_STATE_HOME",
                    "XDG_CACHE_HOME", "XDG_RUNTIME_DIR", "XDG_CONFIG_DIRS", "XDG_DATA_DIRS"):
            path = directory / key
        ...
        for command in commands:
            result = subprocess.run([str(artifact), *command], env=env, cwd=directory,
                                    capture_output=True, text=True, timeout=15, check=True)
            initializers = [line for line in result.stderr.splitlines()
                            if "running initializer" in line.lower()]
            if not initializers:
                raise RuntimeError("missing dyld initializer trace; cannot verify delayed startup")
            if any(framework in line for line in initializers for framework in FRAMEWORKS):
                raise RuntimeError(f"{command[0]} eagerly initialized a network framework")
            ...
            if command[0] == "config" and json.loads(result.stdout)["rules"] != ["MATCH,DIRECT"]:
                raise RuntimeError("dump lost the literal fixture rule")
            print(f"PASS cold {command[0]}: {len(initializers)} initializers, no native network framework")

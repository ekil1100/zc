...
def main():
    ...
    output = (args.output or ROOT / f"target/reliability/soak-{time.time_ns()}.json").resolve()
    if output == ROOT or ROOT / "docs" in output.parents:
        parser.error("write exploratory output under target/ or a temporary directory, not docs/")
    ...
    subprocess.run(["cargo", "build", "--locked", "--release", "--bin", "zc", "--target-dir", "target"], cwd=ROOT, check=True)
    source_after = source_hashes()
    report = {"schema_version": 1, "formal_baseline": False, "kind": "reliability", "scope": args.scenario,
              "status": "FAIL", "duration_hours": args.hours, "requested_seconds": duration,
              "historical_soak_complete": False, "mixed_port": args.port, "measurements": [],
              "crashes": 0, "port_failures": 0,
              "provenance": {"source_files_sha256": source_after, "source_changed_during_build": source_before != source_after, "binary": str(binary), "binary_sha256": hashlib.sha256(binary.read_bytes()).hexdigest(),
                             "subject_commit": subprocess.check_output(["git", "rev-parse", "HEAD"], cwd=ROOT, text=True).strip(),
                             "worktree_status": subprocess.check_output(["git", "status", "--porcelain", "--untracked-files=all"], cwd=ROOT, text=True)},
              "omitted": ["DNS timeout injection", "proxy failover", "hot reload rollback", "historical throughput/latency thresholds"]}
    child = None
    start = None
    signal.signal(signal.SIGTERM, interrupted)
    signal.signal(signal.SIGINT, interrupted)
    try:
        with tempfile.TemporaryDirectory(prefix="zc-soak-") as work, Server(("127.0.0.1", 0), Echo) as origin:
            ...
            report["provenance"]["launched_binary_sha256"] = hashlib.sha256(binary.read_bytes()).hexdigest()
            env = dict(os.environ)
            for key, directory in [("HOME", "home"), ("XDG_CONFIG_HOME", "config"), ("XDG_RUNTIME_DIR", "run"), ("XDG_STATE_HOME", "state"), ("XDG_CACHE_HOME", "cache")]:
                path = work / directory
            ...
            if args.config:
                ...
                result = subprocess.run([binary, "config", "dump", "-c", config, "--no-override", "--json"], env=env, capture_output=True, text=True, timeout=30, check=True)
                document = json.loads(result.stdout)
                if document.get("external-controller") or document.get("allow-lan") or document.get("bind-address") not in (None, "127.0.0.1"):
                    raise RuntimeError("soak config must be loopback-only with no external controller")
            else:
                config.write_text(f"mixed-port: {args.port}\nallow-lan: false\nrules:\n  - MATCH,DIRECT\n")
            ...
            with log_path.open("w") as log:
                def launch():
                    return subprocess.Popen([binary, "start", "--foreground", "-c", config, "--port", str(args.port)], env=env, stdout=log, stderr=log)
                ...
                try:
                    ...
                    while True:
                        if child.poll() is not None:
                            raise RuntimeError(f"foreground process exited: {child.returncode}")
                        try:
                    ...
                    if args.scenario == "process-exit":
                        ...
                        while True:
                            if child.poll() is not None:
                                raise RuntimeError("restarted foreground process exited")
                            try:
                                forward(args.port, origin.server_address[1])
                                break
                            except (OSError, RuntimeError):
                                if time.monotonic_ns() - recovery_start > 30_000_000_000:
                                    raise RuntimeError("process recovery exceeded 30 seconds")
                                time.sleep(0.05)
                        report["recover_time_ms"] = (time.monotonic_ns() - recovery_start) / 1e6
                        report["recover_actions"] = ["harness restarted owned foreground child", "verified CONNECT echo"]
                    while True:
                        sample = {"elapsed_seconds": time.monotonic() - start, "alive": child.poll() is None, "forward_ok": False}
                        if not sample["alive"]:
                            report["crashes"] += 1
                    ...
                    if report["crashes"] == 0 and report["port_failures"] == 0 and report["elapsed_seconds"] >= duration:
                        report["status"] = "PASS"
                finally:
                    stop(child)
                    child = None
        ...
        report["elapsed_seconds"] = report.get("elapsed_seconds", 0 if start is None else time.monotonic() - start)

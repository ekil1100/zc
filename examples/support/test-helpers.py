...
class Helpers(unittest.TestCase):
    ...
    def test_origin_request_reject_and_reserve(self):
        ...
        for args, status, marker in [
            ((), b"200 OK", "REQUEST"),
            (("reject",), b"403 Forbidden", "REJECT"),
        ]:
            ...
            self.assertEqual(
                p.stdout.readline().strip(), f"E2E_ORIGIN_{marker}=/nonce-1"
            )
            self.assertIn(
                b"zc-e2e-origin:nonce-1" if not args else b"forbidden", response
            )
    ...
    def test_obfs_exact_length_and_half_close_attestation(self):
        for mode in ["fragmented_header", "same_write_tail"]:
            ...
            self.assertEqual(
                ...
                f"E2E_OBFS_ORACLE_EXPECTED=oracle:host=alias.test:body=3:mode={mode}",
            )
            ...
            for count, length in enumerate([2, 4], 1):
                ...
                self.assertEqual(
                    p.stdout.readline().strip(),
                    f"E2E_OBFS_ORACLE_RAW_ACCEPTED=oracle:{count}",
                )
                self.assertEqual(
                    p.stdout.readline().strip(),
                    f"E2E_OBFS_ORACLE_REJECTED=oracle:raw={count}:verified=0:error=ContentLengthMismatch",
                )
            received = []
            ...
            self.assertTrue(
                response.startswith(b"HTTP/1.1 101 Switching Protocols\r\n")
            )
            self.assertTrue(response.endswith(b"\r\n\r\nTAIL"))
            self.assertEqual(
                p.stdout.readline().strip(), "E2E_OBFS_ORACLE_RAW_ACCEPTED=oracle:3"
            )
            self.assertEqual(
                p.stdout.readline().strip(), "E2E_OBFS_ORACLE_VERIFIED=oracle:1"
            )
            self.assertEqual(
                p.stdout.readline().strip(),
                "E2E_OBFS_ORACLE_REQUEST=oracle:GET_HOST_UPGRADE_CONNECTION_KEY_CONTENT_LENGTH_EXACT:3",
            )
            self.assertEqual(
                p.stdout.readline().strip(), f"E2E_OBFS_ORACLE_RESPONSE=oracle:{mode}"
            )
            self.assertEqual(
                p.stdout.readline().strip(),
                "E2E_OBFS_ORACLE_FORWARD=oracle:RAW_TCP_HALF_CLOSE_PASS",
            )
    ...
    def test_obfs_survives_idle_between_harness_phases(self):
        ...
        self.assertEqual(
            p.stdout.readline().strip(),
            f"E2E_OBFS_ORACLE_EXPECTED=idle:host=alias.test:body={len(body)}:mode=same_write_tail",
        )
        ...
        with socket.create_connection(("127.0.0.1", port), timeout=3) as s:
            ...
            self.assertEqual(s.recv(1), b"")
        self.assertEqual(
            p.stdout.readline().strip(), "E2E_OBFS_ORACLE_RAW_ACCEPTED=idle:1"
        )
        self.assertEqual(
            p.stdout.readline().strip(),
            "E2E_OBFS_ORACLE_REJECTED=idle:raw=1:verified=0:error=ContentLengthMismatch",
        )
        ...
        self.assertEqual(
            p.stdout.readline().strip(), "E2E_OBFS_ORACLE_RAW_ACCEPTED=idle:2"
        )
    ...
    def test_udp_independent_vectors_health_and_packet_recovery(self):
        output = subprocess.check_output(
            [str(BIN / "e2e_ss_udp_oracle"), "selftest"], text=True, timeout=10
        )
        ...
        port = int(p.stdout.readline().strip().split(":")[1])
        output = subprocess.check_output(
            [str(BIN / "e2e_ss_udp_oracle"), "health", str(port), "test"],
            text=True,
            timeout=3,
        )
        self.assertEqual(output.strip(), "E2E_SS_UDP_HEALTH_PASS=test")
        wire = bytes.fromhex(
            "f0e0d0c0b0a090807060504030201000fcdb69b1dd02167b6b5fb4e6aa16d75c8656108cc2879ee2a02bee68cf79ad110458a788b1b9f090df"
        )
        with socket.socket(socket.AF_INET, socket.SOCK_DGRAM) as s:
            ...
            with self.assertRaises(socket.timeout):
                s.recv(65536)
            self.assertEqual(
                p.stdout.readline().strip(), "E2E_SS_UDP_ORACLE_RAW=test:1"
            )
            s.settimeout(3)
            for count, kind in [(2, "BAD_TAG"), (3, "NORMAL")]:
                ...
                for marker in [
                    f"RAW=test:{count}",
                    f"VERIFIED=test:{count}",
                    f"RESPONSE=test:{count}:{kind}",
                ]:
                    self.assertEqual(
                        p.stdout.readline().strip(), "E2E_SS_UDP_ORACLE_" + marker
                    )
        for family, af, ip in [
            ...
            self.assertEqual(
                p.stdout.readline().strip(), f"E2E_UDP_ECHO_PACKET={family}:1"
            )

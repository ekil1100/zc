...
| R28 UNSUPPORTED_PROXY_TYPE_CHECK | error/warn | 代理类型不支持：未知类型（snell 等）为 error；http/socks5/socks 出站为 warn（解析接受、v1.0 出站未实现，运行时连接失败） | `type: snell` | 更换为 ss/vmess/trojan/vless/anytls |

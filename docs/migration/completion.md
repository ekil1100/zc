...
`35250040916 / 5bcd9a2` 是独立诊断，**不是验收通过**：

- 不启动 Rust 的系统 User trust 写入仍失败，keychain-only 对照成功；关联的 authd/SecurityAgent 记录已证实该请求进入会话所有者认证并检测到确认对话框。下一步需要能完成合法认证的专用一次性 macOS 环境，不能修改授权策略或删掉信任用例。
- 原生 Intel 最小链路 270 次 probe 与两次原样 core（trace=0/1）均通过；所有样本保留，原 UDP 超时未复现，**没有据此宣布修复**。
- 正式 CI `35247844266` 另出现 Intel `restart` 的文件安全检查拒绝，尚未定位，与 UDP 分开追踪。

方法、证据路径及限制见[平台诊断](../reliability/platform-diagnostics.md)。本轮只补充测试诊断，没有修改生产重试、超时、校验或本机安装。
...
2. **原生与可靠性证据未齐**：旧候选四平台通过不替代当前 macOS TrustSettings 门禁；系统信任写入已关联到真实系统认证要求，但合法授权环境及信任正/负矩阵尚未完成；Intel UDP 旧失败未复现，另有公开 restart 文件安全检查拒绝待定位。24/72h 长稳、完整 RSS/尾延迟与受支持 OS 矩阵未全部证明。

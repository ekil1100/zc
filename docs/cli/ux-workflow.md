...
- JSON 字段名：`action`、`state`、`pid`、`uptime_seconds`、`mixed_port`、`active_config`、`selected_proxies`、`paths`、`daemon_state`、`proxy_reachable`、`group`、`proxy`、`source`。`selected_proxies[].source` 为 `persisted`、`transient` 或 `default`，不会把仅存在于当前进程内存的选择标成已持久化。`mixed_port` 是运行中 daemon 的有效 mixed listener 端口（显式 `--port` 或默认 `7899`）；停止态为 `null`。

...
| `zc start [-c <config>] [--port <port>] [--foreground] [--json]` | `zc up` | Fork-and-exit daemon start. `--foreground` runs without forking (containers/systemd). Background start fully prepares providers/overrides in the parent, then launches from an owner-only authenticated snapshot. Already-running daemon is reported as success (`detail:"already_running"`), exit 0. `--port` overrides mixed-port for this run. 配置已有非零 `mixed-port` 时，同文件中的 `port`/`socks-port` 按 ignored 语义处理，不创建额外 listener；没有 mixed listener 的 standalone 声明仍在 bind 前拒绝。Override flags (`--override-script`/`--override-arg`/`--override-timeout-ms`) accepted. |

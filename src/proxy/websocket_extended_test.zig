...
test "decodeExtended64 uses all 8 bytes without target-width truncation" {
    // 0x01_00000000 = 2^32. A 64-bit target can represent it; a 32-bit
    // target must reject it instead of silently using only the low 4 bytes.
...
test "decodeExtended64 accepts the largest target-representable protocol length" {
    const protocol_max: u64 = std.math.maxInt(i63);
    const target_max: u64 = @intCast(std.math.maxInt(usize));
    const expected_u64 = @min(protocol_max, target_max);
    var ext: [8]u8 = undefined;
    std.mem.writeInt(u64, &ext, expected_u64, .big);
    const got = try WebSocket.decodeExtended64(ext);
    try testing.expectEqual(@as(usize, @intCast(expected_u64)), got);
}
...
test "WebSocket upgrade request format" {
    ...
    const written = try std.fmt.bufPrint(&request, "GET {s} HTTP/1.1\r\n" ++
        "Host: {s}\r\n" ++
        "Upgrade: websocket\r\n" ++
        "Connection: Upgrade\r\n" ++
        "Sec-WebSocket-Key: {s}\r\n" ++

...
- select proxy groups;
- DIRECT、REJECT、`aes-128-gcm` / `aes-256-gcm` / `chacha20-poly1305` / `chacha20-ietf-poly1305` Shadowsocks TCP（plain 或内置 simple-obfs HTTP），以及 Trojan TCP/TLS 出站；
- minimal REST control endpoints.

Shadowsocks 现在精确支持 plain AEAD TCP，或 `plugin: obfs|obfs-local` + `plugin-opts: {mode: http, host: <1..255 bytes>}`；`plugin_opts` map 会在输出时规范化为 `plugin-opts`。simple-obfs `tls`、未知 mode、通用外部插件、缺失/不安全 host 与 `udp: true` 均在 bind/dial 前拒绝，绝不回退为裸 Shadowsocks。HTTP、SOCKS5、VMess、VLESS、AnyTLS outbound 与 Shadowsocks UDP 仍未支持。其他已知缺口包括完整 mihomo DNS、TUN/redir/tproxy、完整 REST API v1、WebSocket 事件流和第三方 dashboard 兼容。
